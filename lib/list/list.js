...
export default class UVList extends LitElement {
  ...
  scrollToItem(index) {
    ...
    const start = this.itemsMap[item.id]?.start ?? null;
  ...
  navUp() {
    ...
    const record = this.itemsMap[item.id];
    if (record.start < this.wrapperRef.value.scrollTop) {
  ...
  navDown() {
    ...
    if (
      record &&
      record.end > this.wrapperRef.value.scrollTop + this.wrapperSize
    ) {
      this.wrapperRef.value.scrollTop = record.end - this.wrapperSize;
    }
