...
export default class UVList extends LitElement {
  ...
  constructor() {
    ...
    this.navKeys = () => [...["ArrowUp", "ArrowDown"], ...(this.vimNavigation ? ["j", "k"] : [])]
    this.navUp = () => [...["ArrowUp"], ...(this.vimNavigation ? ["k"] : [])]
    this.navDown = () => [...["ArrowDown"], ...(this.vimNavigation ? ["j"] : [])]
  }
  ...
  firstUpdated() {
    ...
    document.addEventListener("keydown", (event) => {
      ...
      if (this.navKeys().includes(event.key)) {
        ...
        if (this.selectedIndex === 0 && this.wrapperRef.value.scrollTop > 0) {
          this.wrapperRef.value.scrollTop = 0;
          return;
        }
        if (this.navUp().includes(event.key)) {
        ...
        if (this.navDown().includes(event.key)) {
          if (this.selectedIndex === this.items.length - 1) {
            if (
              this.wrapperRef.value.scrollTop !==
              this.wrapperRef.value.scrollHeight
            ) {
              this.wrapperRef.value.scrollTop =
                this.wrapperRef.value.scrollHeight;
            }
          ...
          if (
            ...
            // this.wrapperRef.value.scrollTo({
            //   top: record.getEnd() - this.wrapperSize,
            //   behavior: "instant",
            // });
            this.wrapperRef.value.scrollTop =
              record.getEnd() - this.wrapperSize;
          }
          this.dispatchEvent(
            new CustomEvent("selected", {
              composed: true,
