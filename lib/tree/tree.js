...
export default class UVTree extends LitElement {
  ...
  willUpdate(props) {
    if (props.get("searchQuery") !== this.searchQuery) {
      if (this.searchParams.enabled) {
        this._searchMode =
          this.searchQuery?.length >= this.searchParams.minimalQueryLength;
        this.updateInternal();
      }
    }

    if (
      props.get("_searchMode") !== this._searchMode &&
      this._searchMode === false
    ) {
      this._visualMap = {};
      this.updateInternal();
  ...
  updateInternal(initial = false) {
    ...
    console.log("###############", this._searchMode, this.searchQuery);
    if (this._searchMode) {
