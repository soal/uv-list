...
export default class UVTree extends LitElement {
  ...
  updateVisibleList(initial = false) {
    ...
    for (let i = 0; i < this._filteredList.length; i++) {
      const item = this._filteredList[i];
  ...
  hideRecursive(item) {
    ...
    for (let i = 0; i < item.children.length; i++) {
      const child = this._processed[item.children[i]];
      if (child?.children?.length) {
        this._visualMap[item.children[i]] = false;
  ...
  renderTreeItem(item, index, selected) {
    ...
    // const style = styleMap({ "padding-left": padding })
    const style = `padding-left: ${padding}`
  ...
  scrollToTop() {
    this.listRef.value?.scrollToTop();
  }
