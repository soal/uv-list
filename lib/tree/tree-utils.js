...
export function addToList(items, key, idList, itemList) {
  ...
  if (item.track.length > 1) {
    const parentIndex = idList.indexOf(String(item.track[item.track.length - 1]));
  ...
  const index = idList.indexOf(String(key));
  if (index > -1) {
    return;
  }
  ...
  if (item.children && item.children.length) {
    ...
    for (const id of item.children) {
      addToList(items, String(id), idList, itemList);
    }
  }
}
/**
 ...
 */
export function makeDefaultPredicate({ searchString, params = { fields: ['name'] } }) {
  return item =>
